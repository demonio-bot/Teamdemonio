//NO PASES EL GITH A NADIE PUTA
//GRACIAS POR EMBALAR 
// THANKS TO DEMONIO AND ALL CREATOR BOT SPANISH
...
const ig = require('insta-fetcher')// SC NYA JANGAN DI JUAL BANG
//MAKASIH DAH PAKE 
...
async function starts() {
	...
	fs.existsSync('./session.json') && demonio.loadAuthInfo('./session.json')
	demonio.on('connecting', () => {
	console.log(color('> [ INGFO ]', 'white'), color('Connecting...'))
	})
	demonio.on('open', () => {
	console.log(color('> [ INGFO ]', 'white'), color('Connected'))
	})
		await demonio.connect({timeoutMs: 30*1000})
  fs.writeFileSync('./session.json', JSON.stringify(demonio.base64EncodedAuthInfo(), null, '\t'))
//Banned Call
demonio.on('CB:action,,call', async json => {
    const callerId = json[2][0][1].from;
    console.log("call dari "+ callerId)
        demonio.sendMessage(callerId, "Auto block system, don't call please", MessageType.text)
        await sleep(4000)
        await demonio.blockUser(callerId, "add")
})
//Welkom
	demonio.on('group-participants-update', async (anu) => {
		if (!welkom.includes(anu.jid)) return
		try {
            num = anu.participants[0]
            const mdata = await demonio.groupMetadata(anu.jid)
            try {
                var pp_user = await demonio.getProfilePicture(num)
            } catch (e) {
                var pp_user = 'https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png?q=60'
            }
            if (anu.action == 'add') {
                ini_user = demonio.contacts[num]
                ini_img = await getBuffer(`http://api.lolhuman.xyz/api/welcomeimage?apikey=${LolHuman}&img=${pp_user}&text=${ini_user.notify}`)
                group_info = await demonio.groupMetadata(anu.jid)
                welkam = `Hola ${ini_user.notify}\n◪ Welcome in group:\n├─ ${mdata.subject}\n│\n├─ Intro dulu\n├─ ❏ Nama: \n├─ ❏ Umur: \n├─ ❏ Asal kota: \n├─ ❏ Kelas: \n├─ ❏ Jenis kelamin: \n└─ ❏ Nomor: ${num.replace('@s.whatsapp.net', '')}\nSemoga Betah yaa~~\n${ini_user.notify}`
                demonio.sendMessage(anu.jid, ini_img, MessageType.image, { caption: welkam })
            }
            if (anu.action == 'remove') {
                ini_user = demonio.contacts[num]
                ini_img = await getBuffer(`http://api.lolhuman.xyz/api/welcomeimage?apikey=${LolHuman}&img=${pp_user}&text=${ini_user.notify}`)
                out = `◪ Goodbye ${ini_user.notify}\n◪ Leave from group:\n${mdata.subject}\n│\n└─ ❏ Nomor: ${num.replace('@s.whatsapp.net', '')}\nGoodBye~~`
                demonio.sendMessage(anu.jid, ini_img, MessageType.image, { caption: out })
            }
		} catch (e) {
...
//
		const totalchat = await demonio.chats.all()
		const groupMetadata = isGroup ? await demonio.groupMetadata(from) : ''
		...
		mess = {
			wait: 'ᵉˢᵖᵉʳᵃ ᵖᵉʳʳᵃ,ⁿᵒ ʰᵃᵍᵃˢ ˢᵖᵃᵐ ᵖᵘᵗᵃ 👿',
			success: 'ᶜᵒᵐᵃⁿᵈᵒ ᶜᵒⁿ ᵉˣⁱᵗᵒ,ᵈᵉᵐᵒⁿⁱᵒ ᵈᵃᵈᵈʸ🔥',
			wrongFormat: 'ᵉˢᶜʳⁱᵇᵉ ᵇⁱᵉⁿ ᵉˡ ᶠᵒʳᵐᵃᵗᵒ ᵖᵘᵗᵃ',
			error: {
				stick: 'ᵛᵃˡⁱˢᵗᵉ ᵛᵉʳᵍᵃ 📴 ⁿᵒ ˢᵉ ᵖᵘᵈᵒ ᶜᵒⁿᵛᵉʳᵗⁱʳ ᵃ ˢᵗⁱᶜᵏᵉʳ',
				Iv: 'ᵛᵃˡⁱˢᵗᵉ ᵛᵉʳᵍᵃ ❌ ˡⁱⁿᵏ ⁱⁿᵛᵃˡⁱᵈᵒ'
			},
			only: {
        ...
        const reply = (teks) => {
            demonio.sendMessage(from, teks, text, {quoted:mek})
        }

        const sendMess = (hehe, teks) => {
            demonio.sendMessage(hehe, teks, text, {sendEphemeral: true})
        }
...
demonio.sendMessage(from, teks, MessageType.text, {
quoted: {
key: {
fromMe: false,
...
'vcard': `BEGIN:VCARD\nVERSION:3.0\nN:XL;${ucapanWaktu},;;;\nFN:${ucapanWaktu},\nitem1.TEL;waid=${sender.split('@')[0]}:${sender.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`,
 'jpegThumbnail': fs.readFileSync('./stik/thumb.jpeg')
        ...
        const fakethumb = (teks, yes) => {
            demonio.sendMessage(from, teks, image, {thumbnail:fs.readFileSync('./stik/fake.jpeg'),quoted:mek,caption:yes})
        }
        const fakestatus = (teks) => {
            demonio.sendMessage(from, teks, text, {
                quoted: {
                    key: {
                        fromMe: false,
	  ...
	  contextInfo:{
        ...
        const sendStickerFromUrl = async(to, url) => {
                ...
                download(url, './stik' + names + '.png', async function () {
                    ...
                    exec(`ffmpeg -i ${filess} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${asw}`, (err) => {
                        ...
                        demonio.sendMessage(to, media, MessageType.sticker,{quoted:mek})
        ...
        const sendMediaURL = async(to, url, text="", mids=[]) =>{
                ...
                download(url, filename, async function () {
                    ...
                    demonio.sendMessage(to, media, type, { quoted: mek, mimetype: mime, caption: text,contextInfo: {"mentionedJid": mids}})
...
╭╊━┈ै✾※҉═᪳͢►ۣۜ⌝ːː⌜◄᪳͢═※✾ै┈━╉╮
   ...
   ᴰᵉ͢ᵐᵒⁿⁱᵒ¿?͓-❤️-ᴰᵉ͢ᵐᵒⁿⁱᵃღ¿?͓✨❤️ᵈᵃʳʸ ᵐⁱ ᵈᵘᵉñᵃ ᵗᵉ ᵃᵐᵒ ᵇᵉᵇᵉ❤️✨
...
├┉↯ *${prefix}runtime*
├┉↯*${prefix}speed*
╰──┉↯ 💔⃞⃝⃟⃢👑🔥ˢⁱᵐᵖ ᵈᵉ ᵈᵉᵐᵒⁿⁱᵃ🔥💔⃞⃝⃟⃢☠️⃝⃞⃟⃢🏴‍☠️
`
 ...
 //Set Owner For gc
    case prefix+ 'public':
          	...
          	break
					case prefix+ 'peson':
					  demonio.toggleDisappearingMessages(from, WA_DEFAULT_EPHEMERAL)
					  break
					  case prefix+ 'pesoff':
					    demonio.toggleDisappearingMessages(from, 0)
					    break
            case prefix+ 'spam':
                ...
                demonio.sendMessage(from, argz[0], MessageType.text, {sendEphemeral: true})
                }
	        break
             case prefix+ 'mute':
                if (!isMe) return reply('This command only for saia')
                demonio.modifyChat(from, ChatModification.mute, 24*60*60*1000)
                reply('*succes mute this chat*')
                console.log('succes mute chat = ' + from)
                break
            case prefix+ 'unmute':
                if (!isMe) return reply('This command only for saia')
                demonio.modifyChat(from, ChatModification.unmute)
            ...
            case prefix+ 'delete':
                ...
                demonio.modifyChat(from, ChatModification.delete)
                break
	...
	case prefix+ 'setfakeimg':
	  if (isMe) return('Khusus Sayah bang kamu siapa?')
			...
			delb = await demonio.downloadMediaMessage(boij)
...
//INFORMATION
    ...
    case prefix+ 'cnn':
                ...
                demonio.sendMessage(from, buff, MessageType.image, {caption: cn})
                break
    case prefix+ 'infogempa':
                ...
                demonio.sendMessage(from, thumbbb, MessageType.image, {caption: `${captt}`})
                break
//tools
     case prefix+ 'get':
      if(!q) return reply('linknya?')
			...
			demonio.sendMessage(from, buffqw, image, {quoted: mek})
			.catch(err => {
    ...
    case prefix+ 'revip':
			...
			demonio.sendMessage(from, `${repip}`, text)
      .catch((error) => {
...
case prefix+ 'avatar':
	...
	case prefix+ 'husbu':
			...
			demonio.sendMessage(from, nye, image, { caption: 'Gatau caption nya apa', quoted: mek })
			.catch(err => {
				return('Mengulang lord ada yg mengerror...')
			})
			break
...
case prefix+ 'image':
            ...
            demonio.sendMessage(from,{url:images},image,{quoted:mek})
            });
            break
case prefix+ 'pinterest':
			...
			ke =  sasu[Math.floor(Math.random() * sasu.length)];
			nye = await getBuffer(ke)
			demonio.sendMessage(from, nye, image, { caption: 'Powered bai gatau pinterest lah bang awokwok', quoted: mek })
			.catch(err => {
    ...
    case prefix+ 'anime':
            ...
            demonio.sendMessage(from,media,image,{quoted:mek,caption:'NIH'})
            }
            )
            .catch((error) => {
            console.log(error); 
	...
	case prefix+ 'wallpaperanime':
			...
			demonio.sendMessage(from, bufwanime, image, { quoted: mek })
			.catch(err => {
...
case prefix+ 'nsfwavatar':
			anu = await axios.get('https://nekos.life/api/v2/img/nsfw_avatar')
				...
				demonio.sendMessage(from, nsavatar, image, {quoted: mek})
			.catch(err => {
			return('Ad yg mengerror ulang coba..')
			})	
			break
      ...
      rano = getRandom('.webp')
			...
			exec(`wget ${anu.data.url} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
			  fs.unlinkSync(ranp)
				...
				demonio.sendMessage(from, buffer, MessageType.sticker, {quoted: mek})
				fs.unlinkSync(rano)
			})
			break
case prefix+ 'pussy':
...
case prefix+ 'pussyimage':
  pusiimg = await axios.get('https://nekos.life/api/v2/img/pussy_jpg')
			bufpusy = await getBuffer(pusiimg.data.url)
				demonio.sendMessage(from, bufpusy, MessageType.image, {quoted: mek})
			.catch(err => {
			return('Ad yg mengerror ulang coba..')
			})
			break
case prefix+ 'oppai':
			...
			demonio.sendMessage(from, opaiz, image, { quoted: mek })
			.catch(err => {
			return('Ad yg mengerror ulang coba..')
			})
			break
...
case prefix+ 'ero':
			...
			demonio.sendMessage(from, bufero, image, { quoted: mek })
			.catch(err => {
			return('Ad yg mengerror ulang coba..')
			})
			break
case prefix+ 'erokemo':
			...
			demonio.sendMessage(from, erokzs, image, { quoted: mek })
			.catch(err => {
			return('Ad yg mengerror ulang coba..')
			})
			break
case prefix+ 'eroyuri':
			...
			demonio.sendMessage(from, buferoyu, image, { quoted: mek })
			.catch(err => {
			return('Ad yg mengerror ulang coba..')
			})
			break
...
case prefix+ 'femdom':
      anu = await axios.get('https://nekos.life/api/v2/img/femdom')
			bupemdom = await getBuffer(anu.data.url)
				demonio.sendMessage(from, bupemdom, image, {quoted: mek})
			.catch(err => {
			return('Ad yg mengerror ulang coba..')
			})
			break
case prefix+ 'futanari':
			...
			demonio.sendMessage(from, futanz, image, { quoted: mek })
			break
case prefix+ 'les':
      ranp = getRandom('.gif')
      rano = getRandom('.webp')
...
case prefix+ 'trap':
			...
			demonio.sendMessage(from, traps, image, { quoted: mek })
			.catch(err => {
			return('Ad yg mengerror ulang coba..')
			})
			break
...
case prefix+ 'blowjob':
			...
			demonio.sendMessage(from, bufblowz, image, {quoted: mek})
			.catch(err => {
			return('Ad yg mengerror ulang coba..')
			})
			break
	case prefix+ 'hentai':
			...
			demonio.sendMessage(from, bufhtz, image, {quoted: mek})
			.catch(err => {
			return('Ad yg mengerror ulang coba..')
			})
			break
	case prefix+ 'hololewed':
			...
			demonio.sendMessage(from, hololewx, image, { quoted: mek })
			.catch(err =>{
...
case prefix+ 'lewd':
			...
			demonio.sendMessage(from, buflewd, image, { quoted: mek })
			
			break
	case prefix+ 'lewdk':
			lewdkk = await axios.get('https://nekos.life/api/v2/img/lewdk')
			lewdkz = await getBuffer(lewdkz.data.url)
			demonio.sendMessage(from, lewdkz, image, { quoted: mek })
			.catch(err =>{
			  return('Tobat puasa goblokk..')
			})
			break
...
case prefix+ 'yuri':
			...
			demonio.sendMessage(from, bupyuri, image, { quoted: mek })
			.catch(err => {
			return('Ad yg mengerror ulang coba..')
			})
			break
...
case prefix+ 'eron':
			...
			demonio.sendMessage(from, buferon, image, { quoted: mek })
			.catch(err => {
			return('Ad yg mengerror ulang coba..')
			})
			break
...
case prefix+ 'keta':
			...
			demonio.sendMessage(from, bufketa, image, { quoted: mek })
			.catch(err => {
			return('Ad yg mengerror ulang coba..')
			})
			break
...
case prefix+ 'cumimage':
			...
			demonio.sendMessage(from, bupjpg, image, { quoted: mek })
			.catch(err => {
			return('Ad yg mengerror ulang coba..')
			})
			break
...
case prefix+ 'holoero':
		holox = await axios.get('https://nekos.life/api/v2/img/holoero')
			...
			demonio.sendMessage(from, bufholox, image, { quoted: mek })
			.catch(err => {
			return('Ad yg mengerror ulang coba..')
			})
			break
//MAKER
    ...
    case prefix+ 's':
            ...
            const media = await demonio.downloadAndSaveMediaMessage(encmedia)
                ran = '666.webp'
                await ffmpeg(`./${media}`)
                .input(media)
                ...
                .on('end', function () {
                console.log('Finish')
                demonio.sendMessage(from, fs.readFileSync(ran), MessageType.sticker, {quoted: mek})
                 fs.unlinkSync(media)
                fs.unlinkSync(ran)
                })
    ...
    case prefix+ 'swm':
            ...
            const encmedia = isQuotedImage   ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
             media = await demonio.downloadAndSaveMediaMessage(encmedia)
            await createExif(a,b)
            out = getRandom('.webp')
            ffmpeg(media)
            .on('error', (e) => {
            console.log(e)
            demonio.sendMessage(from, 'Terjadi kesalahan', 'conversation', { quoted: mek })
            fs.unlinkSync(media)
            })
            .on('end', () => {
            _out = getRandom('.webp')
            ...
            .on('exit', () => {
            demonio.sendMessage(from, fs.readFileSync(_out),'stickerMessage', { quoted: mek })
            fs.unlinkSync(out)
            fs.unlinkSync(_out)
            fs.unlinkSync(media)
            })
    ...
    case prefix+ 'fdeface':
            ...
            const tipes = await demonio.downloadAndSaveMediaMessage(dipes)        
    		...
    		var mat = await demonio.generateLinkPreview(url)
    		mat.title = title;
    		mat.description = desc;
    		mat.jpegThumbnail = bufer;
   			mat.canonicalUrl = buu; 
    		demonio.sendMessage(from, mat, MessageType.extendedText, anu)
            break
case prefix+ 'emoji':
...
case prefix+ 'attp':
						...
						demonio.sendMessage(from, attp2, MessageType.sticker, {quoted: mek})
						break
//MAKERIMAGE
//COMINGSOON
//CONVERTER
    ...
    case prefix+ 'tovideo':
            ...
            owgi = await demonio.downloadAndSaveMediaMessage(ger)
    ...
    case prefix+ 'tomp3':
            ...
            demonio.sendMessage(from, buffer453, audio, { mimetype: 'audio/mp4', quoted: mek })
            fs.unlinkSync(ran)
            })
            break
    case prefix+ 'fast':
            ...
            if (err) return fakegroup(`Err: ${err}`)
            buffer453 = fs.readFileSync(ran)
            demonio.sendMessage(from, buffer453, video, { mimetype: 'video/mp4', quoted: mek })
            fs.unlinkSync(ran)
            })
            break
    case prefix+ 'slow':
...
//Upload Story WeA
    case prefix+ 'upswteks':
            ...
            demonio.sendMessage('status@broadcast', `${q}`, extendedText)
    ...
    case prefix+ 'upswvideo':
            ...
            var bot = gh.split("|")[2];
            demonio.sendMessage(from, `${bot}`, text, {quoted: { key: { fromMe: false, participant: `${mentioned}`, ...(from ? { remoteJid: from } : {}) }, message: { conversation: `${target}` }}})
            break
//MOBIL LEJEN
...
//Downloader
	...
	case prefix+ 'ytsearch':
        	...
        	return await demonio.sendMessage(from, 'Error!', MessageType.text, dload)
    		}
    		aramat = aramas.all 
    ...
    case 'ig':
	    ...
	    demonio.sendMessage(from,igvdl,video,{mimetype:'video/mp4',quoted:mek,caption:'Nih'})
            } else if (ink.includes('.jpg')){
            const igpdl = await getBuffer(ink)
            demonio.sendMessage(from,igpdl,image,{mimetype:'image/jpeg',quoted:mek,caption:'Nih'})
	    }
            }
	    })
	    break
    ...
    case prefix+ 'twitter':
 	...
 	case prefix+ 'tiktok':
		...
		tik.ssstik(`${args[0]}`)
    		...
    		.then(async (a) => {
    		me = `*Title* : ${text}\n*Link* : ${a.data}`
		demonio.sendMessage(from,{url:`${videonowm}`},video,{mimetype:'video/mp4',quoted:mek,caption:me})
		})
		})
    ...
    case prefix+ 'tiktokaudio':
		...
		demonio.sendMessage(from,{url:`${music}`},audio,{mimetype:'audio/mp4',filename : `${text}`,quoted:mek})
		})
     		.catch(e => console.log(e))
     		break
    case prefix+ 'fb':
...
case prefix+ 'brainly':
			...
			demonio.sendMessage(from, teks, text,{quoted:mek,detectLinks: false})                        
            })              
			break
case prefix+ 'google':
      if(!q) return reply('Yang mau di cari afa?')
			...
			demonio.sendMessage(from, bupnyah, image, {quoted: mek, sendEphemeral: true})
			.catch(err => {
				return('Mengulang lord ada yg mengerror...')
			})
			break
...
demonio.groupRemove(from, mentioned)
					} else {
...
demonio.groupMakeAdmin(from, mentioned)
}
break
case prefix+ 'demote':
if (!isGroup) return reply(mess.only.group)
...
demonio.groupDemoteAdmin(from, mentioned)
}
break//DONE
    ...
    case prefix+ 'kontak':
            ...
            + 'END:VCARD'.trim()
            demonio.sendMessage(from, {displayName: `${nah}`, vcard: vcard}, contact)
            break
 	case prefix+ 'hidetag':
			if (!mek.key.fromMe) return fakestatus('SELF-BOT')
			if (!isGroup) return reply(mess.only.group)
			var value = args.join(' ')
			var group = await demonio.groupMetadata(from)
			...
			demonio.sendMessage(from, optionshidetag, text)
			break
    case prefix+ 'sticktag':
            if ((isMedia && !mek.message.videoMessage || isQuotedSticker) && args.length == 0) {
            encmedia = isQuotedSticker ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
            file = await demonio.downloadAndSaveMediaMessage(encmedia, filename = getRandom())
            value = args.join(" ")
            var group = await demonio.groupMetadata(from)
            var member = group['participants']
            var mem = []
            ...
            demonio.sendMessage(from, ini_buffer, MessageType.sticker, options)
            fs.unlinkSync(file)
            } else {
		    ...
		    case prefix+ 'giftag':
		      //by Dehanjing
                ...
                download = await demonio.downloadMediaMessage(quoted)
                ...
                demonio.sendMessage(from, thumb, video, { contextInfo: {mentionedJid: mem }, quoted: mek, mimetype: 'video/gif', thumbnail: thumb })
			...
			case prefix+ 'doctag':
			  //by Dehanjing
                ...
                demonio.sendMessage(from, fs.readFileSync(`doc.txt`), document, { contextInfo: {mentionedJid: mem }, quoted: mek, mimetype: 'text/plain' })
    ...
    case prefix+ 'kontag':
            ...
            + `TEL;type=CELL;type=VOICE;waid=${entah}:${phoneNum('+' + entah).getNumber('internasional')}\n`
            + 'END:VCARD'.trim()
            demonio.sendMessage(from, {displayName: `${nah}`, vcard: vcard}, contact, {contextInfo: {"mentionedJid": members_ids}})
            break
    case prefix+ 'totag':
            if ((isMedia && !mek.message.videoMessage || isQuotedSticker) && args.length == 0) {
            encmedia = isQuotedSticker ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
            ...
            ini_buffer = fs.readFileSync(file)
            demonio.sendMessage(from, ini_buffer, image, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            demonio.sendMessage(from, ini_buffer, video, options)
            fs.unlinkSync(file)
        } else{
...
//OTHER 
    case prefix+ 'join':
            ...
            var response = await demonio.acceptInvite(codeInvite)
            fakestatus('SUKSES')
            } catch {
...
case prefix+ 'bc':
					...
					anu = await demonio.chats.all()
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						bc = await demonio.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							demonio.sendMessage(_.jid, bc, image, {caption: `${body.slice(4)}\n> Izin Broadcast`, sendEphemeral: true})
						}
...
.group)
				...
				case prefix+ 'narutod':
				oi = body.slice(8)
				oii = await getBuffer(`https://demonioapi.herokuapp.com/api/photooxy/naruto?text=${oi}`)
				demonio.sendMessage(from, oii, image, {quoted: mek, caption: 'Ok brother'})
				break
